...
class CPPCHECKLIB CheckUnusedStruct : public Check {
public:
    /** This constructor is used when registering the CheckUnusedStruct */
    CheckUnusedStruct() : Check(myName()) {
    }

    /** This constructor is used when running checks. */
    CheckUnusedStruct(const Tokenizer *tokenizer, const Settings *settings, ErrorLogger *errorLogger)
        : Check(myName(), tokenizer, settings, errorLogger) {
    }

    void runSimplifiedChecks(const Tokenizer *tokenizer, const Settings *settings, ErrorLogger *errorLogger) {
        CheckUnusedStruct checker(tokenizer, settings, errorLogger);
...
private:
    void getErrorMessages(ErrorLogger *errorLogger, const Settings *settings) const {
        CheckUnusedStruct c(0, settings, errorLogger);
    }
